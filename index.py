...
# getting user input
    ...
    suddenlweightloss = col1.selectbox("Do you currently feel sudden weight loss?",["Yes","No"])
    ...
    visualblurring = col1.selectbox("Do you have visual blurring?",["Yes","No"])

    Itching = col2.selectbox("Do you have Itching?",["Yes","No"])

    Irritability = col1.selectbox("Do you have Irritability?",["Yes","No"])

    delayedhealing = col2.selectbox("Do you feel any delayed healing?",["Yes","No"])
    ...
    model = joblib.load('C:/Users/vidur/Desktop/2136903-Artifact/diab_model.pkl')
    ...
    if st.button('Predict'):
        ...
        if(prediction[0]==0):
            st.write('<p class="big-font">You likely will not develop diabetes disease in 10 years.</p>',unsafe_allow_html=True)

        else:
            st.write('<p class="big-font">You are likely to develop diabetes disease in 10 years.</p>',unsafe_allow_html=True)
...
# getting user input
    ...
    CholCheck = col3.selectbox("Have you ever checked cholestrol?",["Yes", "No"])
    ...
    PhysActivity = col2.selectbox("Do you have Phsycal activities?",["Yes","No"])

    Fruits = col3.selectbox("Do you eat Fruits?",["Yes","No"])

    Veggies = col1.selectbox("Do you eat Veggies?",["Yes","No"])

    HvyAlcoholConsump = col2.selectbox("Do you use Alchohol?",["Yes","No"])

    AnyHealthcare = col3.selectbox("Do you have any health activities?",["Yes","No"])

    NoDocbcCost = col1.selectbox("Do you have no doccbcost?",["Yes","No"])
    ...
    model = joblib.load('C:/Users/vidur/Desktop/2136903-Artifact/heart_model.pkl')
    ...
    if st.button('Predict'):
        ...
        if(prediction[0]==0):
            st.write('<p class="big-font">You do not have posibility to develop heart disease in 10 years.</p>',unsafe_allow_html=True)

        else:
            st.write('<p class="big-font">You  have the posibility to develop heart disease in 10 years.</p>',unsafe_allow_html=True)
...
# getting user input
    ...
    model = joblib.load('C:/Users/vidur/Desktop/2136903-Artifact/thal_model.pkl')
    ...
    if st.button('Predict'):
        ...
        if(prediction[0]==0):
            st.write('<p class="big-font">You are not an Alpha Thalassemia carrier.</p>',unsafe_allow_html=True)

        else:
            st.write('<p class="big-font">You are an Alpha Thalassemia carrier.</p>',unsafe_allow_html=True)
